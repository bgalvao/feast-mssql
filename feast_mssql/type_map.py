...
def arrow_to_mssql_type(t_str: str) -> str:
    try:
        if t_str.startswith("timestamp") or t_str.startswith("datetime"):
            return "DATETIMEOFFSET" if "tz=" in t_str else "DATETIME"
        return {
            "null": "null",
            "bool": "bit",
            "int8": "tinyint",
...
def mssql_type_to_feast_value_type(type_str: str) -> ValueType:
    type_map: Dict[str, ValueType] = {
        ...
        "nvarchar(4000)": ValueType.STRING,
...
def mssql_type_code_to_mssql_type(code: int) -> str:
    return {
        16: "boolean",
        17: "bytea",
        20: "bigint",
...
def mssql_type_code_to_arrow(code: int) -> str:
    return feast_value_type_to_pa(
        mssql_type_to_feast_value_type(mssql_type_code_to_mssql_type(code))
    )
