...
from feast_mssql.connection_utils import _get_conn, df_to_mssql_table, get_query_schema
from feast_mssql.mssql_config import MSSQLConfig
from feast_mssql.offline_stores.mssql_source import MSSQLSource
from feast_mssql.type_map import mssql_type_code_to_arrow


class MSSQLOfflineStoreConfig(MSSQLConfig):
...
class MSSQLOfflineStore(OfflineStore):
    ...
    def pull_latest_from_table_or_query(
        ...
        assert isinstance(data_source, MSSQLSource)
        ...
        query = f"""
            ...
            FROM (
                ...
                FROM ({from_expression}) AS a
                WHERE a.{timestamp_field} BETWEEN (CAST('{start_date}' AS datetimeoffset)) AND (CAST('{end_date}' AS datetimeoffset))
            ) AS b
            WHERE _feast_row = 1
            """
    ...
    def get_historical_features(
        ...
        def query_generator() -> Iterator[str]:
            ...
            if isinstance(entity_df, pd.DataFrame):
                ...
                entity_schema = df_to_mssql_table(
            ...
            try:
                ...
                if table_name:
                    with _get_conn(config.offline_store) as conn, conn.cursor() as cur:
                        # TODO: does this require a conn.commit?
    ...
    def pull_all_from_table_or_query(
        ...
        query = f"""
            ...
            WHERE "{timestamp_field}" BETWEEN (CAST('{start_date}' AS datetimeoffset)) AND (CAST('{end_date}' AS datetimeoffset))
        """
    ...
    def _to_arrow_internal(self) -> pa.Table:
        with self._query_generator() as query:
            with _get_conn(self.config.offline_store) as conn, conn.cursor() as cur:
                ...
                cur.execute(query)
                fields = [
                    # likely to return a KeyNotFoundError...
                    (c[0], mssql_type_code_to_arrow(c[1]))
...
def build_point_in_time_query(
    ...
    """Build point-in-time query between each feature view table and the entity dataframe for MSSQL"""
